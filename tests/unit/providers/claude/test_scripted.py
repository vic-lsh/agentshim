"""The scripted Claude Code stream round-trips through the real parser."""
...
def _agent(executor: FakeExecutor, recorder: RecordingEventHandler | None = None) -> CliAgent:
    return CliAgent("claude", executor=executor, env=dict(_ENV), event_handler=recorder)
...
class TestScriptedTurn:
    ...
    def test_a_second_turn_resumes_the_first(self) -> None:
        executor = FakeExecutor(
            [
                scripted_turn("claude", text="ok", session_id="s1"),
                scripted_turn("claude", text="juniper", session_id="s1"),
            ]
        )
        ...
        assert executor.requests[1].argv[1:3] == ["--resume", "s1"]
    ...
    def test_tool_calls_become_paired_events(self) -> None:
        recorder = RecordingEventHandler()
        run = scripted_turn("claude", text="done", tool_calls=[("Bash", {"cmd": "ls"}, "file.txt")])
    ...
    def test_usage_round_trips_exactly(self) -> None:
        ...
        result = _agent(FakeExecutor(scripted_turn("claude", text="done", usage=usage))).run("go")
    ...
    def test_structured_output_round_trips_on_a_schema_turn(self, tmp_path: Path) -> None:
        payload = {"answer": 4}
        executor = FakeExecutor(scripted_turn("claude", structured_output=payload))
        request = TurnRequest(
            prompt="what is 2 + 2?",
        ...
        assert "--json-schema" in executor.requests[0].argv

    def test_a_nonzero_return_code_is_scripted(self) -> None:
        assert scripted_turn("claude", text="done", returncode=2).returncode == 2
